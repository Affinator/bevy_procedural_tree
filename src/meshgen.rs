...
pub fn generate_tree_meshes(
    settings: &TreeMeshSettings,
    rng: &mut Rng,
) -> Result<(Mesh, Mesh), BevyError> {
...
fn generate_branches_internal(
    ...
    recurse_a_branch(
        ...
        state,
        rng,
        &mut branches_attributes,
        &mut leaves_attributes,
    )?;

    // build meshes
    let mut branches_mesh = Mesh::new(
...
fn recurse_a_branch(
    ...
    #[cfg(not(feature = "u32_indices"))]
    if branches_attributes.indices.len()
        >= (u16::MAX as usize - approx_amount_of_indices_of_this_branch)
    {
        return Err(IndicesOverflowError.into());
    }
    #[cfg(feature = "u32_indices")]
    if branches_attributes.indices.len()
        >= (u32::MAX as usize - approx_amount_of_indices_of_this_branch)
    {
        return Err(IndicesOverflowError.into());
    }
    ...
    let factor_for_length: f32 = if state.level > 0 {
        1.0
    } else {
        match settings.tree_type {
            TreeType::Deciduous => {
                (settings.branch.levels as usize - state.recursion_count + 1) as f32 / target_pieces
            }
    ...
    let taper_amount_per_section = match settings.tree_type {
        TreeType::Deciduous => f32::powf(
            1.0 - state.taper.clamp(0.0, 0.9999),
            (1.0 / state.sections as f32) / (f32::from(settings.branch.levels) + 1.0),
        ),
        TreeType::Evergreen => f32::powf(
            1.0 - state.taper.clamp(0.0, 0.9999),
            1.0 / state.sections as f32,
        ),
    };
    ...
    for section_counter in 0..=state.sections {
        ...
        if section_counter == state.sections
            && !((state.level == 0) && matches!(settings.tree_type, TreeType::Deciduous))
        {
        ...
        if section_counter < state.sections {
            ...
            let radius_factor =
                1.0 - (section_radius / settings.branch.force.radius_cutoff).clamp(0.0, 1.0);
            let strength_per_radius =
                (settings.branch.force.strength * radius_factor / 2.0).clamp(0.0, 1.0); // 2 chosen by trial and error to look natural (values between 0..1 make the most sense now; larger is still possible)
...
fn generate_child_branches(
    ...
    for i in 0..count {
        ...
        let child_start_factor =
            f32::lerp(settings.branch.start[level].clamp(0.0, 1.0), 1.0, rng.f32());
        ...
        let child_branch_origin = section_a
            .origin
            .lerp(section_b.origin, branch_height_factor);
        ...
        let parent_orientation = section_b
            .orientation
            .slerp(section_a.orientation, branch_height_factor);

        // calculate needed angles
        let radial_angle =
            2.0 * std::f32::consts::PI * (radial_offset + (i as f32) / (count as f32));
...
fn generate_leaves(
    ...
    #[cfg(not(feature = "u32_indices"))]
    if leaves_attributes.indices.len()
        >= (u16::MAX as usize - approx_amount_of_indices_of_this_leaf)
    {
        return Err(IndicesOverflowError.into());
    }
    #[cfg(feature = "u32_indices")]
    if leaves_attributes.indices.len()
        >= (u32::MAX as usize - approx_amount_of_indices_of_this_leaf)
    {
        return Err(IndicesOverflowError.into());
    }
    ...
    for i in 0..settings.leaves.count {
        ...
        let parent_orientation = section_b
            .orientation
            .slerp(section_a.orientation, leaf_height_factor);

        // calculate needed angles
        let radial_angle = 2.0
            * std::f32::consts::PI
            * (radial_offset + (i as f32) / (settings.leaves.count as f32));
...
fn generate_leaf(
    ...
    for rotation in rotations.iter() {
        ...
        .into_iter()
        .map(|v| (leaf_orientation * v + origin).to_array())
        .collect();
        ...
        leaves_attributes.indices.extend_from_slice(&[
            indices_start,
            indices_start + 1,
            indices_start + 2,
            indices_start,
